...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/28 20:57:28 by cmartini          #+#    #+#             */
/*   Updated: 2020/01/28 22:56:38 by cmartini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void		init_juliamx(t_main *m_struct)
{
	MX = 0.0;
	MY = 0.0;
	CR = -0.7;
...
static void		fractal_loop(t_main *m_struct, int x, int y, int i)
{
	while (NEW_X * NEW_X + NEW_Y * NEW_Y < 4 && i > 1)
	{
		OLD_X = NEW_X;
		OLD_Y = NEW_Y;
		NEW_X = fabs(OLD_X * OLD_X - OLD_Y * OLD_Y + CR);
		NEW_Y = fabs(2 * OLD_X * OLD_Y + CI);
		--i;
		m_struct->pixels[y * WIDTH + x] = (i << 21) + (i << 10) + i * 8;
	}
}

static int		juliamx(t_main *m_struct)
{
	int	y;
	int	x;
	int	max_iter;
